...
module.exports.recvFile = (hosts, filename, cb)=>{
    let 
};

module.exports.delFile = (hosts, filename, cb)=>{
    let checked = 0;
    let arr = [];
    hosts.forEach((host, idx)=>{
        const conn = net.createConnection(host.port, host.ip, ()=>{
            ...
            conn.on('data', data =>{
                ...
                if(++checked >= hosts.length){
                    if(arr.length == 0 ) cb(undefined);
                    else cb(arr);
...
module.exports.recvFileMaster = (conn, dir) =>{
    ...
    conn.on('close',()=>{
        ...
        console.log(`Download Successed : ${fileName}`)
    })
};
